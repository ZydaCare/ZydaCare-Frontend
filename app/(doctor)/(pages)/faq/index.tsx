...
import { getFAQs, getFeaturedFAQs, getPopularFAQs, FAQItem } from '@/api/doctor/faq';
  ...
  const fetchFAQs = async () => {
    try {
      ...
      const [faqsResponse, featuredResponse, popularResponse] = await Promise.all([
        getFAQs(undefined, undefined, 'doctor'),
        getFeaturedFAQs('doctor', 3),
        getPopularFAQs('doctor', 5)
      ]);
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <View className="px-6 pt-6 pb-4 bg-white border-b border-gray-100">
        <View className="flex-row items-center justify-between mb-2">
          <Text className="text-3xl font-sans-bold text-gray-900">FAQ Center</Text>
          <TouchableOpacity onPress={() => router.push('/(doctor)/(pages)/help')}>
            <View className="w-10 h-10 bg-gray-100 rounded-full items-center justify-center">
              <Ionicons name="help-circle-outline" size={24} color="#67A9AF" />
            </View>
          </TouchableOpacity>
        </View>
        ...
        <View className="px-6 pb-8">
          ...
          <View className="mt-6 mb-6 bg-gradient-to-br from-primary/10 to-primary/5 rounded-2xl p-6 border border-primary/20">
            ...
            <TouchableOpacity 
              ...
              className="bg-primary px-6 py-3.5 rounded-xl flex-row items-center justify-center shadow-sm"
            >
