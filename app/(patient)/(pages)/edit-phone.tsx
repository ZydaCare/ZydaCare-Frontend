...
import { Animated, KeyboardAvoidingView, Modal, Platform, SafeAreaView, ScrollView, StatusBar, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
export default function EditPhoneScreen() {
    ...
    const [phone, setPhone] = useState(() => {
        // Remove the country code from the saved phone number if it exists
        const savedPhone = user?.phone || '';
        return savedPhone.replace(/^\+?234/, '').replace(/\s+/g, '');
    });
    ...
    const formatPhoneNumber = (text: string) => {
        // Remove all non-numeric characters and the country code if present
        let cleaned = text.replace(/\D/g, '');

        // Remove the country code if it's included
        if (cleaned.startsWith(countryCode.replace(/\D/g, ''))) {
            cleaned = cleaned.substring(countryCode.replace(/\D/g, '').length);
        }
    ...
    const handlePhoneChange = (text: string) => {
        ...
        // Update the state with the formatted number (without country code)
        setPhone(formatted);
    };
    // const handlePhoneChange = (text: string) => {
    //     const formatted = formatPhoneNumber(text);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Only send the local part of the phone number (without country code)
                ...
                >
                    ...
                    <View className="mt-auto">
                        <TouchableOpacity
                            ...
                            activeOpacity={0.8}
                            className={`rounded-full py-4 items-center shadow-lg ${!phone.trim() || phone.replace(/\s/g, '').length < 10
                                ? 'bg-gray-300'
                                : isSaving
                                    ? 'bg-secondary/70'
                                    : 'bg-secondary'
                        ...
                        >
                            <View className="flex-row items-center">
                                {isSaving ? (
                                    ...
                                    <Text className={`font-sans-bold text-base ${!phone.trim() || phone.replace(/\s/g, '').length < 10 ? 'text-gray-500' : 'text-white'
                                        }`}>
                                        Save Changes
                                    </Text>
                                )}
                            ...
                            <ScrollView className="flex-1">
                                {countryCodes.map((country) => (
                                    <TouchableOpacity
                                        ...
                                        className={`flex-row items-center p-4 border-b border-gray-50 ${country.code === countryCode ? 'bg-primary/5' : ''
                                            }`}
                                        activeOpacity={0.7}
                                    >
